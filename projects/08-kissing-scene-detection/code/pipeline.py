    ...
    def one_video_extract_audio_and_stills(cls,
        ...
        while cap.isOpened():
            ...
            if frame_id % math.floor(frame_rate * params.vggish_frame_rate) == 0:
                ...
                # images += [transformer(frame_pil) for _ in range(self.n_augment)]  # 数据增强：复制图像n_augment次

        cap.release()

        # 处理音频
        # TODO: 解决OpenMP错误的临时方法
        os.environ['KMP_DUPLICATE_LIB_OK'] = 'TRUE'

        tmp_audio_file = 'tmp.wav'
        VideoFileClip(path_video).audio.write_audiofile(tmp_audio_file)
        # 如果n_augment > 1，则通过复制每个样本n_augment次来修复
        audio = vggish_input.wavfile_to_examples(tmp_audio_file)
        # audio = audio[:, None, :, :]  # 为"channel"添加虚拟维度
        # audio = torch.from_numpy(audio).float()  # 将输入示例转换为float类型
        ...
        # images = [torch.from_numpy(img).permute((2, 0, 1)) for img in images[:min_sizes]]  # 将图像通道从HWC转换为CHW
