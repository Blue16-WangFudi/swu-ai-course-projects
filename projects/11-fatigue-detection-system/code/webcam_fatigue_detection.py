...
USE_WEBCAM = True  # 必须为 True
CAMERA_ID = 0  # 0 = 默认前置摄像头
SAVE_OUTPUT = False  # 是否保存录像
...
face_3d_model = np.array([
    ...
    (-225.0, 170.0, -135.0),    # 左眼外角 (263)
    (225.0, 170.0, -135.0),     # 右眼外角 (33)
    (-150.0, -150.0, -125.0),   # 左嘴角 (291)
    (150.0, -150.0, -125.0)     # 右嘴角 (61)
], dtype=np.float64)
...
print(" 启动实时疲劳检测...")
print(f" 请正对摄像头，前 {INIT_DURATION_SEC} 秒保持睁眼以完成校准！")
...
while True:
    ...
    if results.multi_face_landmarks:
        ...
        if not is_initialized and frame_count < INIT_FRAMES:
            ...
            cv2.putText(frame, f"[CALIBRATING... {frame_count}/{INIT_FRAMES}]",
        ...
        # 检测阶段
            ...
            if not is_initialized and open_eye_heights:
                ...
                is_initialized = True
                print(f"校准完成！参考睁眼高度 = {ref_eye_height:.1f}px")
            ...
            if is_initialized and ref_eye_height is not None:
                ...
                for idx in [386, 374, 159, 145]:
                    x = int(lm[idx].x * w)
                    y = int(lm[idx].y * h)
                    cv2.circle(frame, (x, y), 2, (0, 255, 255), -1)  # 眼部：青色
                for idx in [13, 14]:
                    x = int(lm[idx].x * w)
                    y = int(lm[idx].y * h)
                    cv2.circle(frame, (x, y), 2, (0, 255, 0), -1)  # 嘴部：绿色
    ...
    # 显示信息
    if is_initialized:
        cv2.putText(frame, f"Eye Ref: {ref_eye_height:.1f}px", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        cv2.putText(frame, f"Closed Eye Frms: {consecutive_eye_closed:.1f}", (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        cv2.putText(frame, f"Mouth Ht: {mouth_height:.1f}px", (20, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        cv2.putText(frame, f"Open Mouth Frms: {consecutive_mouth_open:.1f}", (20, 140), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        cv2.putText(frame, f"Pitch: {pitch:.1f}°", (20, 170), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        cv2.putText(frame, f"Head Down Frms: {consecutive_head_down:.1f}", (20, 200), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)

    cv2.putText(frame, status, (20, 240), cv2.FONT_HERSHEY_SIMPLEX, 1.0, color, 2)
...
if SAVE_OUTPUT and out:
    out.release()
    print(f"录像已保存: {os.path.abspath(OUTPUT_VIDEO)}")
